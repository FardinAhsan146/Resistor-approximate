...
def equivalent_tol(components: List[float], target: float, series: bool, tolerance: float, resistor: bool) -> List[float]:
    ...
    condition = (not resistor and not series) or (series and resistor)
    _target = target if condition else 1/target
    _components = components if condition else [1/x for x in components]
